...
class Net(object):
  ...
  def construct_net(self, hr_images, lr_images):
    ...
    self.loss = self.softmax_loss(self.conditioning_logits, labels)
...
#模型建立
xs = tf.placeholder(float, [None, 28, 28, 1])
ys = tf.placeholder(float, [None, 7, 7, 1])
net = Net(xs, ys, 'prsr')
...
optimizer = tf.train.RMSPropOptimizer(learning_rate_decay, decay=0.95, momentum=0.9, epsilon=1e-8)
